...
def map_polys(polys, numbers):
    map_list = [];
    #map each poly over the x sequence
    for tup in polys:
        map_list.append(list(map(lambda x: tup(x) , numbers)))
    return map_list

def display_poly_maps(poly_maps):
    #list unpacking to display what we want
...
def make_nth_deg_rand_poly(n, a, b):
    ...
    #I make a list of the random integer coefficients to use for the poly
    i = 0
    rand = []
    while i < n + 1:
        rand.append(gen_rand_coeff(a,b))
        i += 1
    #I don't want this to display for each degree (p(x) = cx^n, p(x) = cx^n-1 .. etc) so I put it in the list first
    # and then what I return will join each individual element together on ''
...
def sorted_rand_poly_dance(num_polys, n, a, b, xvals):
    ...
    #the second element of each tuple is the value we want to sort by, .sort() will
    #sort the list in place
    concat_list.sort(key=lambda x: x[1], reverse = True)
    return concat_list

#this function is to display the sorted list of polys since I
#changed the list of lists of tuples to a list of tuples
...
if __name__ == '__main__':
    start = datetime.now()
    top_10 = sorted_rand_poly_dance(10000,5,1,10,range(0,3))[:10]
    end = datetime.now()
    duration = end = start
    print('start = ', start)
    print('end = ', end)
